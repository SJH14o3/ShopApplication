            ...
            public void handle(ActionEvent event) {
                ...
                if(emailValidate(su_EmailField.getText()) && confirm == 0 ){
                    ...
                    if (!su_UsernameField.getText().trim().isEmpty() && !su_EmailField.getText().trim().isEmpty() && !su_PasswordField.getText().trim().isEmpty() &&
                        ...
                        String userType;
                        if (su_ConsumerButton.isSelected()) {
                            userType = "Consumer";
                        }
                        else userType = "Vendor";
                        database.SignUpUser(event, su_UsernameField.getText(), su_EmailField.getText(), su_PasswordField.getText(), userType , CompanyField.getText());
                    } else {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setContentText("Fill all information");
                        alert.show();
...
//                }
    ...
    private void toggleCompanyField() {
        if (su_ConsumerButton.isSelected() && !CompanyField.isDisable()) {
            CompanyField.setDisable(true);
            CompanyField.setText(null);
        }
        else if (su_VendorButton.isSelected() && CompanyField.isDisable()) {
